...
$hookmanager->initHooks(array('doligamelist'));
...
llxHeader('', $langs->trans('doligameList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->doligame->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'doligame t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'doligame_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('doligame', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_doligame', 'GET');
...
$listViewConfig = array(
	...
	,'list' => array(
		'title' => $langs->trans('doligameList')
		,'image' => 'title_generic.png'
		,'picto_precedent' => '<'
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('Nodoligame')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => doligame::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
