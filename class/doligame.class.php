...
class doligame extends SeedObject
{
    /**
     * Canceled status
     */
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'doligameStatusShortCanceled'
		,self::STATUS_DRAFT => 'doligameStatusShortDraft'
		,self::STATUS_VALIDATED => 'doligameStatusShortValidated'
//		,self::STATUS_REFUSED => 'doligameStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'doligameStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'doligame';

	/** @var string $element Name of the element (tip for better integration in Dolibarr: this value should be the reflection of the class name with ucfirst() function) */
	public $element = 'doligame';
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->DOLIGAME_REF_MASK) ? $conf->global->DOLIGAME_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'doligame', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showdoligame") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/doligame/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='doligame@doligame';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('doligameStatusCancel'); $statusLabelShort=$langs->trans('doligameStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('doligameStatusDraft'); $statusLabelShort=$langs->trans('doligameStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('doligameStatusValidated'); $statusLabelShort=$langs->trans('doligameStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('doligameStatusRefused'); $statusLabelShort=$langs->trans('doligameStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('doligameStatusAccepted'); $statusLabelShort=$langs->trans('doligameStatusShortAccepted'); }
...
//class doligameDet extends SeedObject
//{
//    public $table_element = 'doligamedet';
//
//    public $element = 'doligamedet';
//
//
//    /**
//     * doligameDet constructor.
//     * @param DoliDB    $db    Database connector
//     */
//    public function __construct($db)
//    {
